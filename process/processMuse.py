...
class ProcessMuse(IProcess):
  ...
  def __init__(self, sampling_rate, chan=0, attach_plot=False):
    ...
    EEG_chan = SignalBuffer(sampling_rate, window_length = 5, attach_plot=attach_plot, name="rawEEG")
    
    self.morlet = Morlet(EEG_chan, window_length=5, attach_plot=attach_plot, name="the green morlet")
